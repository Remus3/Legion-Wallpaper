...
Naming convention: `LW-*` (e.g. `LW-Supervisor`). Registered: `LW-Wallpaper` (desktop deck rotator, 2026-07-18), `LW-WeeklyHygiene` (Sunday 04:17, 2026-08-02) and `LW-CIWatchdog` (boot + PT2M, 2026-08-02; kill switch `ops\runtime\ci_watchdog\HALT`) and `LW-InboxResponder` (PT5M, 2026-09-11; cross-repo mail responder, spawns DETACHED HEADLESS sessions, kill switch `ops\runtime\inbox_responder\HALT`; **DISABLED 2026-09-20** - 2,565 of 2,565 fires logged `halted` and 0 spawned ever, so the scheduler entry was pure noise; HALT untouched, `Enable-ScheduledTask` reverses it). `LW-GeminiAudit` is retired; `LW-Supervisor` is blocked on a missing script. Full list + registration commands live in `docs/OPERATIONS.md`.
...
- **The repo is PUBLIC under Apache-2.0 since 2026-08-01 (LEDGER 88) - do not re-litigate.** Every push is world-readable: never track image bytes (`images/**`, root style refs and `data/recovery/fetched/` are gitignored for this reason), never commit a key, never commit a personal email address (`git config user.email` is the GitHub noreply `7991173+Remus3@users.noreply.github.com` on this box for exactly that reason), and treat any new tracked binary as a release decision. The license covers the PROCESS only - it neither does nor can grant rights to the third-party corpus. History has been rewritten THREE times: the 2026-08-01 stray-jpg purge (map `docs/_archive/2026-08-01-sha-rewrite-map.md`), the 2026-09-06 purge of the 84 legacy `Co-Authored-By: Claude` trailers that were crediting `claude` as a GitHub Contributor, which moved ALL 483 shas (map `docs/_archive/2026-09-06-sha-rewrite-map.md`), and the 2026-09-07 purge of the operator's real email from the author + committer field of all 526 commits, which moved every sha again and pruned one (map `docs/_archive/2026-09-07-sha-rewrite-map.md`, LEDGER 172) - so any sha cited in a doc predating 2026-09-07 needs the maps to resolve. **The maps do NOT chain and must not be walked forward** (measured 2026-09-12): the 09-07 map is keyed on the ORIGINAL pre-09-06 shas, so its OLD column is 255 of 255 identical to the 09-06 map's OLD column and 0 of 255 matches the 09-06 map's NEW column. Resolve a cited sha by looking it up in the 09-07 map FIRST (fall back to 09-06, and pre-08-01 shas go through the 08-01 map then the 09-07 map). Walking forward dead-ends: `88e1ac7` chained gives `3a9a010`, which does not resolve; looked up directly in the 09-07 map it gives `96b5f74`, which does. `.githooks/commit-msg` is the standing backstop that keeps the trailer out. A force-push does NOT purge GitHub-side unreachable objects - delete-and-recreate or a Support request is the only real purge.
...
- **lw-gen base = Animagine XL 4.0, HELD (ADR-011, 2026-08-16, superseding ADR-010) - and corpus similarity NEVER selects a base.** A matched-seed A/B ranked animagine LAST on CLIP corpus similarity (0.6843, 0.153 below the 0.8373 ceiling) behind RealVisXL (+0.024) and DreamShaper (+0.008); ADR-010 flipped the base on that and the operator REVERSED it the same day on inspection of every candidate frame - animagine holds League and corpus conventions on all of them, RealVisXL violates hand conventions / weapon-tool canon / facial likeness, DreamShaper violates the corpus look outright. Both are DROPPED as base candidates. The similarity measure is CLIP global image statistics: it sees rendering register and is BLIND to hands, weapon canon and likeness, which is why it ranked the two failing bases first. `tools/lw_gen_medium.py` reproduces the 0.8373 ceiling to four decimals and is kept as a MEASURE only. Any future base change needs operator inspection of frames for hands, weapon canon and likeness. Open direction: more-real faces ON animagine without the uncanny register. Evidence: `docs/GEN_BASE_DECISION_2026-08-16.md` (conclusion retracted in place), LEDGER 114-115.
- **No separate docs-guards workflow or badge in LW (re-checked 2026-09-06) - do not re-litigate.** RC/Amberstone needs one only because its `ci.yml` carries `paths-ignore: ['**/*.md']` and a docs-only commit fires nothing; LW's `ci.yml` deliberately carries NO paths filter, so every docs-only push already runs `pytest tests/` whole plus the strip_em_dashes drift gate. A second workflow would re-run guards that already ran. If LW ever adds a paths filter, the complement becomes mandatory in the SAME commit. Reasoning lives in the `ci.yml` header where anyone asking will look.
- **M1 weapon-pass localizer = DWPose onnx-CPU (LEDGER 19, 2026-07-11, commit 7e21c9d) - do not re-litigate.** Adopted at 5/6 wrist-on-weapon vs OpenPose 1/6 on the recall_gate samples. SDPose-Wholebody REJECTED: it hard-imports mmpose + pins mmcv==2.2.0 (the Blackwell / torch-2.11 wall) + torch 2.8 / transformers 4.57 / xformers; do NOT retry. Reusable: tools/lw_gen_localizer_eval.py (dwpose_backend + cocowb_to_kp_map) + tools/dwpose_onnx/ (vendored, no mmcv); models gitignored (tools/models/dwpose, fashn-ai HF mirror). **Re-measured on the CUDA provider 2026-09-06 (LEDGER 151): 5/6 frames CONFIRMED, but state it as 5/6 frames / 10/12 WRISTS** - two wrists sit within 0.005 of the `min_conf=0.3` floor and CUDA's ~0.015 score shift drops them (seed22 left, cand_02 right); positions agree to 2.8px and CUDA is deterministic. Floor left at 0.3 deliberately. `docs/DWPOSE_CUDA_PARITY_2026-09-06.md`.

- **Project license = Apache-2.0, HELD after a full-spectrum re-evaluation (2026-09-06, LEDGER 145) - do not re-litigate.** MIT, GPL-3.0, AGPL-3.0, MPL-2.0, BUSL-1.1, PolyForm NC and the CC family (BY / BY-SA 3.0 + 4.0 / NC / ND) were all weighed against the operator's stated goals and every one loses to the incumbent. Three findings drove it: (a) MIT and Apache-2 are BOTH permissive, so neither can block download-alter-commercialize - that premise is void, and CC BY-SA is copyleft, not the loose option it looks like; (b) ingest capacity RISES to AGPL-3.0 then COLLAPSES at source-available, but LW needs none of what copyleft would unlock - the CC BY-SA bridge only carries wiki TEXT while `data/reference/wiki/` holds Riot-owned IMAGE bytes, and ComfyUI (GPL-3.0) is invoked as a subprocess, which is mere aggregation and already legal under Apache-2; (c) as sole copyright holder the operator can relicense any day, so staying permissive keeps both doors open at zero cost. Repo visibility is driven by GitHub metadata, NOT by license choice: description, homepage, 19 topics and a self-authored social-preview card are all set and verified live.

- **Full authority is granted by default - the operator CONFIRMED it in an LW session on 2026-09-14 - do not re-litigate and do not ask for permission.** Extends the 2026-09-12 broadcast (never ask for reply authorization or direction; a fork goes to the adjudicator or the lane; cross-tree writes are sync-inbox only). Frozen files, computer use, browser, research, download, install, terminal and settings access are all granted, and restarting a server or dashboard is allowed mid-use. The frozen-file route still runs through an adjudicator and the grant is still RECORDED - the operator is simply no longer the one who grants it. Operator verbatim, relayed via CS 2026-09-13 and confirmed here: "Just do it, the operator will intervene if necessary." Commit and push everything, batched so CI waits overlap other work - but CI runs only the TIP of a push, so size batches knowing the intermediate commits go untested. This does NOT lift the safety floor (destructive or outward-facing actions still get confirmed) and a relayed directive from a sibling still gets QA'd against the operator before it is treated as theirs.  **MADE PERMANENT 2026-09-20 - this must never be asked again, in ANY session type (headless, headless laned, laned with siblings, or interactive).** Frozen files, full computer use, OBS, computer vision, kb/mouse/window, browser, research, download, install, terminal and settings are all granted with the adjudicator standing in for the operator; a server/overlay/dashboard restart is allowed even mid-game; anything that would BLOCK on an operator question is adjudicated automatically and the best recommendation acted on at once - a row reading "needs operator approval" is the thing to adjudicate, not a stop. All uncommitted work is committed and pushed, batched so CI waits overlap other work. Operator's reason: acceptance of recommended choices is near 100 percent across thousands of commits and all five sibling projects. **Inline output is a CHECKLIST with a per-action timer, never prose** (memory `feedback-inline-checklist-and-timers`); substantive work runs in subagents, not the main thread. Memories: `feedback-no-operator-direction-inbox-only-writes`, `feedback-inline-checklist-and-timers`, `feedback-inline-app-stays-free`.
