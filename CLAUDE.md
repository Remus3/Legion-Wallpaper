...
- **The repo is PUBLIC under Apache-2.0 since 2026-08-01 (LEDGER 88) - do not re-litigate.** Every push is world-readable: never track image bytes (`images/**`, root style refs and `data/recovery/fetched/` are gitignored for this reason), never commit a key, and treat any new tracked binary as a release decision. The license covers the PROCESS only - it neither does nor can grant rights to the third-party corpus. History has been rewritten TWICE: the 2026-08-01 stray-jpg purge (map `docs/_archive/2026-08-01-sha-rewrite-map.md`) and the 2026-09-06 purge of the 84 legacy `Co-Authored-By: Claude` trailers that were crediting `claude` as a GitHub Contributor, which moved ALL 483 shas (map `docs/_archive/2026-09-06-sha-rewrite-map.md`) - so any sha cited in a doc predating 2026-09-06 needs the maps to resolve, newest first. `.githooks/commit-msg` is the standing backstop that keeps the trailer out. A force-push does NOT purge GitHub-side unreachable objects - delete-and-recreate or a Support request is the only real purge.
