...
- Write the initial synopsis to `%USERPROFILE%/Desktop/LW_HEADLESS_SYNOPSIS_<YYYY-MM-DD>.md` (atomic Write). Header carries: HEAD sha, core version constant (TBD until defined), LW test count, CI status, item count, scope, stop rules, phase log table.
- TaskCreate for each phase in the run so progress is visible.
- Init the resumable slice manifest so a crash (API 400 / socket drop / cascade-cancel) never wipes the run: `python tools/slice_orchestrator.py init --run-id <YYYY-MM-DD-NN> --head <sha>`, then `add` one entry per planned slice. The manifest at `ops/runtime/slice_manifest.json` is the durable checkpoint; on a relaunch `python tools/slice_orchestrator.py resume` lists only the non-committed slices to redo. The wrapper `tools/headless_run.ps1` automates relaunch-on-crash + this resume handoff. Both tools EXIST as of 2026-07-29 (LEDGER 60) - do not re-bootstrap them; `init` REFUSES over a manifest holding non-committed slices, which is the anti-clobber guard, so on a relaunch use `resume` (or `init --force` only when deliberately discarding the checkpoint).
...
- CWD HAZARD: the shell CWD persists between Bash calls. Run `cd "C:/Legion Wallpaper"` before each `git merge`, or use `git -C "C:/Legion Wallpaper" merge ...`. A merge fired from a worktree dir lands on the wrong branch.
- **Verifier gate before any merge (ground truth, not the slice agent's word).** A slice agent's "green: N tests pass" is a CLAIM, not a fact - the pipe replays stale results and agents have cited non-existent test files in past runs. Before merging a slice, dispatch the read-only `verifier` subagent (`Agent` tool, `subagent_type: "verifier"`) with the claim + the cited test command + the cited files. It re-runs the suite fresh, confirms the cited files exist, cross-checks counts, and returns CONFIRM or REFUTE. Merge only on CONFIRM; on REFUTE re-dispatch the slice (mark it `failed` in the manifest) - never merge a refuted slice. The verifier has no Edit/Write tools so it cannot mutate anything; it only reports. **Truth-gate (mechanized layer):** for the FINAL pre-commit reconciliation of a multi-slice round, the verifier (or merger) runs `python tools/truth_gate.py --claims <claims.json>` - it re-runs the real suite to a file (stale-pipe-proof), re-reads every claimed file and confirms the claimed CONTENT is present (`must_contain` snippets, not just existence), probes CI for HEAD via `gh run list --commit`, and writes `ops/runtime/truth_gate_report.json` atomically. Exit 2 = REFUSE: commit is BLOCKED, `quarantined` lists the slices to re-dispatch with their discrepancy lines as added context. Exit 0 = PROCEED. (Bootstrap the tool if absent - same rule as the slice manifest.)
- **Claim before you dispatch, and the manifest ENFORCES it**: `python tools/slice_orchestrator.py claim --agent <agent-id> --files <the slice's files> --slice <S>` first, then `set --id <S> --status in_progress --agent <agent-id>`. That `set` is REFUSED unless the same agent holds a claim covering every file the slice declares, and a slice added with no `--files` cannot start at all - so `add` every slice with its real file list. This is the disjointness check: an overlapping claim is refused and the refusal names the holder, instead of the sole merger discovering the overlap after both agents spent their run. Release on the way out with `release --agent <agent-id>`.
- **Checkpoint each slice in the manifest as it advances**: `--status verified` after the verifier CONFIRMs, `--status committed --commit <sha>` after the merge + push land (neither is gated - they record work already done). A crash after a committed mark means that slice is durable and is skipped on resume.
...
1. **Lint locally before push**: `python -m py_compile <touched files>`; if any Python file was edited, also `python -m ruff check .`. F541 (f-string no placeholder) is the most common CI-killer; catch it locally.
2. **Test gate**: run the relevant test subset green BEFORE committing. Full suite: `python -m pytest tests/ -q`. Component-scoped subsets: TBD - define per component as the product grows (mirror the pattern: one pytest command per major component, plus any snapshot/visual suite).
...
- WAKEUP_NOTES prune: `python "C:/Legion Wallpaper/scripts/wakeup_prune.py" --keep 3` (no-op when already <=3 sessions; bootstrap the script if absent). Older sessions archive to `docs/history_notes.md`.
...
### 10b. Context management across sessions (operator directive 2026-07-29)

A long headless run is context-bound, not just time-bound. The run must stay resumable by a FRESH session that has none of this one's context. Two failure modes to design against: (a) the run dies or gets summarized mid-flight and the next session cannot tell what is owed; (b) the run's own doc writes bloat the per-session bootstrap load so every future session pays for this one.

**Protect the bootstrap window.** Session start reads `CLAUDE.md` + `MEMORY.md` + git log + `WAKEUP_NOTES.md` + `docs/ARCHITECTURE.md` + `ROADMAP.md`. That set is the per-session context tax, so anything appended to it is a permanent cost paid by every later session.
- `CLAUDE.md` is CI size-budgeted (< 60KB, `tools/drift_guard.py` warns from 80 percent). NEVER append ledger entries; they go to `docs/LEDGER.md`.
- `ROADMAP.md` is the heaviest of the set in practice and is only advisory-budgeted. Highest priority sits at the TOP and a bootstrap typically reads the top ~80 lines, so a verbose new item silently pushes existing priorities OUT of the window. Honor the file's own stated grammar - `id - title - state - next action - evidence link` - and keep detail in a dated `docs/` artifact that the entry LINKS to. Measured instance: this directive's own run first wrote three items spanning 60 lines and displaced every pre-existing priority; compressing them to the grammar recovered the window.
- Before the final docs commit, measure rather than assume: byte-count the bootstrap set and confirm `ROADMAP.md`'s top items still fit the window. Run `tools/drift_guard.py` and treat a budget note as work, not decoration.
- Do NOT unilaterally prune or gut ANOTHER item to buy room, even one labelled DONE - a closed-looking item often still carries a live `Next:`. Compress what THIS run wrote; propose the rest as a FUTURE item.

**When to `/clear` mid-run.** The orchestrator thread accretes context fastest (every slice report, every verifier verdict). `/clear` between focus-area switches is the standing rule, and it is SAFE ONLY when the run's state lives on disk, never in the thread. Before clearing, confirm all four: the slice manifest is checkpointed (`slice_orchestrator.py status` shows every finished slice `committed`), the Desktop synopsis reflects current state, merged work is pushed, and any owed carry-forward is written down. Then `/clear` and re-bootstrap from disk. If any of those four is not true, the manifest and synopsis are what you fix FIRST - clearing with state only in the thread is how a run loses work.

**Prefer disk over thread for anything load-bearing.** Long tool outputs, censuses and probe results belong in a file (scratchpad for throwaway, `docs/` for durable evidence) and get referenced by path. Never re-read a large artifact into the thread to "confirm" it; cite the path and the number.

**Hand off to the next session explicitly.** `/done` writes `WAKEUP_NOTES.md`, which is what a fresh session actually bootstraps from - so the wakeup entry, not the chat, is the deliverable. It must state: what shipped with SHAs, verified suite/CI numbers, what is OWED, every operator-gated decision the run refused to make on its own, and the do-not-redo set. Then emit an explicit NEXT-SESSION PROMPT in chat: the one-line task, the exact ROADMAP item ids to pick up, the files or docs to read first, and any command to re-run. Assume the next session shares zero context with this one.
